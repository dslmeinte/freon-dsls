...
import { JsonDirectorySyntaxAnalyserPart, JsonCommonSyntaxAnalyserPart } from ".";

/**
 *   Class JsonSyntaxAnalyser is the main syntax analyser.
 ...
 *
 */
export class JsonSyntaxAnalyser implements SyntaxAnalyser {
    sourceName: string = "";
    locationMap: any;
    private _unit_JsonDirectory_analyser: JsonDirectorySyntaxAnalyserPart = new JsonDirectorySyntaxAnalyserPart(this);
    private _unit_common_analyser: JsonCommonSyntaxAnalyserPart = new JsonCommonSyntaxAnalyserPart(this);
    ...
    public transformSharedPackedParseTreeNode(node: SPPTNode): any {
        if (!!node) {
            try {
                ...
                if (e.message.startsWith("Syntax error in ") || e.message.startsWith("Error in JsonSyntaxAnalyser")) {
                    throw e;
                } else {
    ...
    private transformSharedPackedParseTreeBranch(branch: SPPTBranch): any {
        ...
        if ("JsonDirectory" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonDirectory(branch);
        } else if ("JsonFile" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonFile(branch);
        } else if ("JsonString" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonString(branch);
        } else if ("JsonNumber" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonNumber(branch);
        } else if ("JsonUndefined" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonUndefined(branch);
        } else if ("JsonNull" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonNull(branch);
        } else if ("JsonArray" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonArray(branch);
        } else if ("JsonObject" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonObject(branch);
        } else if ("JsonKeyValuePair" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonKeyValuePair(branch);
        } else if ("JsonBoolean" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonBoolean(branch);
        } else if ("JsonValue" == brName) {
            return this._unit_JsonDirectory_analyser.transformJsonValue(branch);
        } else if ("__pi_reference" == brName) {
            return this.transform__pi_reference(branch);
        } else {
            throw new Error(`Error in JsonSyntaxAnalyser: ${brName} not handled for node '${branch?.matchedText}'`);
        }
    }

...
import { JsonModelUnitType, ModelUnitMetaType, JsonFiles } from "../../language/gen";
import { JsonGrammarStr } from "./JsonGrammar";
import { JsonSyntaxAnalyser } from "./JsonSyntaxAnalyser";
import { JsonSemanticAnalyser } from "./JsonSemanticAnalyser";

/**
 *   Class JsonModelUnitReader is a wrapper for the various parsers of
 *   modelunits.
 */
export class JsonModelUnitReader implements PiReader {
    analyser: JsonSyntaxAnalyser = new JsonSyntaxAnalyser();
    parser: LanguageProcessor = Agl.processorFromString(JsonGrammarStr, this.analyser, null, null);
     ...
     */
    readFromString(sentence: string, metatype: ModelUnitMetaType, model: JsonFiles, sourceName?: string): JsonModelUnitType {
        this.analyser.sourceName = sourceName;
        let startRule: string = "";
        // choose the correct parser
        if (metatype === "JsonDirectory") {
            startRule = "JsonDirectory";
        }

        // parse the input
        let unit: JsonModelUnitType = null;
        if (this.parser) {
            try {
                let asm;
            ...
            if (!!model) {
                try {
                    if (model.getUnits().filter(existing => existing.name === unit.name).length > 0) {
                        ...
                        model.addUnit(unit);
                        const semAnalyser = new JsonSemanticAnalyser();
